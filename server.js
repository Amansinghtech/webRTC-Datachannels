...
io.on('connection', (socket) => {
	...
	socket.on('ready', (peerId, peerType) => {
		...
		if (peerId in connections) {
			...
			// Create new peer
			const newPeer = { socketId: socket.id, peerId, peerType, socket }
...
app.post('/answer', (req, res) => {
	const sdp = req.headers.sdp
	console.log(sdp)
	const answer = JSON.parse(Buffer.from(sdp, 'base64'))
	const peer = connections['testPeer1']
	console.log(peer.socketId)
	peer.socket.emit('message', { message: { answer } })
	res.send('Success')
})
